...
{
    global $dbHelper;
    $sql = $dbHelper->select("SELECT * FROM xuong_products_coupon  WHERE code = '$code'");
    if (count($sql) > 0) {
        return false;
    } else {
        return true;
    ...
    if (!isset($_POST["name"]) || empty($_POST['name'])) {
        ...
        if (!empty($_POST['code'])) {
            if (!checkCode($_POST['code'])) {
                ...
                $codeValid = $dbHelper->insert('xuong_products_coupon', array(
...
<body>
    ...
    <div class="container mt-3">
        ...
        <table class="table">
            ...
            $counpons = $dbHelper->select("select * from xuong_products_coupon");
